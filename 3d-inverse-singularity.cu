// Compile with:
// nvcc -O3 -o 3d-inverse-singularity 3d-inverse-singularity.cu -lGL -lGLU -lglut
...
__global__ void init_particles(Particle* particles, curandState* states, int seed) {
    ...
    float theta = curand_uniform(&states[i]) * 2.0f * PI;
    float phi = curand_uniform(&states[i]) * PI;
    float v = 1.002f + curand_uniform(&states[i]) * 0.005f;

    particles[i] = {
        SCHWARZSCHILD_R, theta, phi, v,
...
void display() {
    ...
    for (int i = 0; i < NUM_PARTICLES; ++i) {
        ...
        float x = p.r * sinf(p.phi) * cosf(p.theta);
        float y = p.r * cosf(p.phi);
        float z = p.r * sinf(p.phi) * sinf(p.theta);
        glColor3fv(p.color);
        glVertex3f(x, y, z);
    }
    glEnd();
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Inverse Schwarzschild 3D");
